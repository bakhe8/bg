    ...
    def _apply_gtk_runtime_hint() -> None:
        ...
        for candidate in hint_candidates:
            ...
            if candidate.is_file():
                try:
                    install_root = Path(candidate.read_text(encoding="utf-8").strip())
                except OSError:
                    continue
            else:
...
LETTER_TEMPLATE = "letter_pdf.html"
ASSETS_RELATIVE_PATH = ASSETS_DIR.relative_to(WEB_DIR).as_posix()
...
async def generate_pdf(payload: LetterPayload):
    ...
    html = render_letter_html(data)
    try:
        pdf_bytes = HTML(string=html, base_url=str(WEB_DIR)).write_pdf()
    except Exception as exc:  # pylint:disable=broad-except
...
def render_letter_html(data: dict) -> str:
    template = jinja_env.get_template(LETTER_TEMPLATE)
    context = {
        "bank_name": data.get("bank_name", ""),
        "bank_center": data.get("bank_center", ""),
        "bank_po_box": data.get("bank_po_box", ""),
        ...
        "assets_url": ASSETS_RELATIVE_PATH,
    }
